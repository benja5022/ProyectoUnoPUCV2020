...
int menu(HashTable *tabla){
    ...
    gotoxy(1,20);
    printf("Version especial: para Damian\n");
    ...
    switch(tipo){
        ...
        case(4)://totem
            if(area->reserva_de_oro >= card->coste){
                ...
                if(card->habilidad_totem == 0) activarHabilidadTotem(card,card->habilidad_totem,area);
...
void comenzarAtaque(Area_de_juego* Area_final, carta* card){
    ...
    if(!verificarMuerteDeAliado(card, Area_final)){
        ...
        printf("El aliado no fue eliminado por un talisman");
        system("pause");
        if(card->arma){
